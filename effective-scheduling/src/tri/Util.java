...
public class Util{
	...
	public static double getPrice(int myWh, int LoadOthers){
		...
		int totalCost = (int) Math.pow((totalLoad+100)/1000,2);
		if (totalLoad > 0)			
	...
	public static int[] getTotalLoad(ArrayList<Customer> custLst){
		if (custLst.size()>0) {
			int LEN = custLst.get(0).loads.length;
	...
	public static int[] getTotalLoadPerturbed(ArrayList<Customer> custLst, int NOISERANGE, Random rnd){
		if (custLst.size()>0) {
			int LENTIME = custLst.get(0).loads.length;
			...
			*/
			...
			for (int i=0;i<custLst.size();i++){
				for (int j=0; j<LENTIME; j++){
					totalLoads[j] = totalLoads[j] + custLst.get(i).loads[j] + (int) (rnd.nextDouble()*NOISERANGE);
				}
			}	
